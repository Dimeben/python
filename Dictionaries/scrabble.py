...
We would like to combine these two into a dictionary that would map a letter to its point value. Using a list comprehension and zip, create a dictionary called letter_to_points that has the elements of letters as the keys and the elements of points as the values.
"""
...
7 - Create a variable called brownie_points and set it equal to the value returned by the score_word() function with an input of "BROWNIE".
8 - We expect the word BROWNIE to earn 15 points:
(B + R + O + W + N + I + E)
(3 + 1 + 1 + 4 + 4 + 1 + 1) = 15
Lets print out brownie_points to make sure we got it right.
"""
...
player_to_points = {}
"""
11 - Iterate through the items in player_to_words. 
...
# Function to add a word to a player's list
...
# Updating letter_to_points to handle lowercase inputs
letter_to_points.update(
    {letter.lower(): points for letter, points in letter_to_points.items()}
)
...
# Print updated player_to_words and player_to_points
